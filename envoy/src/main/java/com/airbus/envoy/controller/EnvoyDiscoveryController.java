package com.airbus.envoy.controller;
...
import com.airbus.envoy.model.DiscoveryHost;
import com.airbus.envoy.model.DiscoveryHosts;
...
public class EnvoyDiscoveryController {
	...
	public void deleteHost(@PathVariable("serviceName") String serviceName,
		...
		List<DiscoveryHost> tmp = hosts.get(serviceName);
		if (tmp != null) {
			Optional<DiscoveryHost> optHost = tmp.stream().filter(it -> it.getIpAddress().equals(ipAddress))
					.findFirst();
